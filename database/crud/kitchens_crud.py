...
@connection
async def get_kitchen_with_user_id(session, tg_id: int) -> Kitchen | None:
    kitchen = await session.scalar(
        select(Kitchen)
        .options(selectinload(Kitchen.cities))
...
@connection
async def get_kitchens_with_city(session, city_id:int) -> list[Kitchen] | None:
    kitchens = await session.scalars(
        select(Kitchen).where(Kitchen.city_id == city_id)
    )
    return kitchens
