...
from database.models import User, UserForm
...
@connection
async def update_user_form(session, id: int, form: UserForm):
...
@connection
async def get_user_form(session, id: int):
    user = await session.scalar(select(User).where(User.user_id == id))
    return user.form


@connection
async def get_users_agree(session):
    users = await session.scalars(select(User).where(User.form == UserForm.agree))
    users_id = [item.user_id for item in users]
    await session.commit()
    return users_id
