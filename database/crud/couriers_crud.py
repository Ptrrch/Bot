...
@connection
async def change_courier_counter(session):
    couriers = await session.scalars(select(Courier))
    if couriers:
        for item in couriers:
            item.order_counter = 0
    await session.commit()


@connection
async def get_couriers_from_city(session, city_id: int) ->list[Courier]|None:
    couriers = await session.scalars(select(Courier).where(Courier.city_id == city_id))
    if couriers:
        return couriers

@connection
async def increment_courier_counter(session, user_id: int):
    courier = await session.scalar(select(Courier).where(Courier.user_id == user_id))
    if courier:
        courier.order_counter += 1
        await session.commit()
