...
from Keyboards.Base_kb import contact_kb, main_kb, kitchen_kb, admin_kb
from Keyboards.Kitchen_kb import create_city_for_kitchen_keyboard, KitchenItemCbData, KitchenActions, KitchenMoveCb, \
    KitchenMoveActions
...
async def send_kitchen_result(message: types.Message, data: dict):
    ...
    await message.answer(text="Кухня зарегистрирована", reply_markup=main_kb())
...
async def send_change_kitchen_result(message: types.Message, data: dict):
    city = await get_one_city(data['city_id'])
    text = markdown.text(
        "Так теперь выглядит профиль заведения:",
        "",
    ...
    await message.answer(text=text, reply_markup=kitchen_kb())
    await update_kitchen(data)
...
#     await message.answer(
#         f"Отправьте номер телефона заведения ", reply_markup=contact_kb()
#     )
#
#
...
# @router.message(Kitchen.number, F.contact)
...
@router.callback_query(KitchenMoveCb.filter(F.action == KitchenMoveActions.profile))
async def handle_start_kitchen(call: CallbackQuery, callback_data: KitchenMoveCb, state: FSMContext):
    await state.set_state(UpdateKitchen.id)
    await state.update_data(id = callback_data.kitchen_id)
...
@router.message(UpdateKitchen.number, F.contact, RoleFilter())
async def handle_kitchen_number(message: types.Message, state: FSMContext, role: dict):
    ...
    if role == "Kitchen":
        await send_change_kitchen_result(message, data)
