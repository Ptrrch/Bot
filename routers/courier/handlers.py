...
from Keyboards.Base_kb import contact_kb, main_kb, courier_kb
from Keyboards.Courier_kb import create_city_for_courier_keyboard, CourierActions, CourierItemCbData, CourierOrderCb, \
    CourierProfileCb, CourierProfileActions
...
async def send_courier_result(message: types.Message, data: dict):
    ...
    await message.answer(text="Курьер зарегистрирован ", parse_mode=ParseMode.HTML, reply_markup=main_kb())
...
async def send_change_courier_result(message: types.Message, data: dict):
    city = await get_one_city(data['city_id'])
    text = markdown.text(
        "Так теперь выглядит профиль:",
        "",
    ...
    await message.answer(text=text, parse_mode=ParseMode.HTML, reply_markup=courier_kb())
    await update_courier(data)
...
async def handle_kitchen_title(message: types.Message, state: FSMContext):
    ...
    data = await state.update_data(user_id=idx)
    await state.clear()
...
#         "Укажите номер", reply_markup=contact_kb()
#     )
#
...
#
#
# @router.message(Courier.number, F.contact)
...
@router.callback_query(
    CourierProfileCb.filter(F.action == CourierProfileActions.change)
)
async def handle_start_kitchen(call: CallbackQuery, callback_data: CourierProfileCb, state: FSMContext):
    await state.set_state(UpdateCourier.user_id)
    await state.update_data(user_id=callback_data.courier_id)
    await state.set_state(UpdateCourier.name)
    await call.message.answer(text="Укажите новое имя")
...
async def handle_kitchen_city_id_invalid_content_type(call: CallbackQuery):
    await call.answer()
    await call.message.answer("Выберите город из списка")

@router.message(UpdateCourier.number, F.contact, RoleFilter())
async def handle_kitchen_number(message: types.Message, state: FSMContext, role: dict):
    number = message.contact.phone_number
    data = await state.update_data(number=number)
    await state.clear()
    if role == "Admin":
    ...
    if role == "Courier":
        await send_change_courier_result(message, data)
